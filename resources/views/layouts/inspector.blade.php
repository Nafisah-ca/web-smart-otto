...
<head>
    ...
    <title>@yield('title', 'Portal Inspektor') — Smart Otto</title>
...
<nav class="bg-white border-b border-gray-200 sticky top-0 z-40">
    <div class="max-w-5xl mx-auto px-4 flex items-center justify-between h-14">
        <a href="{{ route('inspector.dashboard') }}" class="flex items-center gap-2">
            ...
            <span class="font-bold text-gray-900">Smart Otto <span class="text-primary-600 text-sm">· Inspektor</span></span>
        </a>
        <div class="flex items-center gap-4">
            <span class="text-sm text-gray-600 hidden sm:block"><strong>{{ auth()->user()->name }}</strong></span>
    ...
    <div class="flex flex-col md:flex-row gap-6">
        <aside class="md:w-52 flex-shrink-0">
            <nav class="card p-3 space-y-1">
                ...
                <a href="{{ route('inspector.tasks.index') }}" class="sidebar-link {{ request()->routeIs('inspector.tasks*') ? 'active' : '' }}">
